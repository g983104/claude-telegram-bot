...
async def send_news_briefing(app):
    ...
    await app.bot.send_message(chat_id=OWNER_ID, text="🌍 오늘의 국제뉴스\n\n" + response.content[0].text)

async def send_economy_briefing(app):
    response = client.messages.create(
        model="claude-haiku-4-5-20251001",
        max_tokens=512,
        messages=[{"role": "user", "content": "이번 주 주요 경제 상황 요약과 주목할 주식 2-3개 추천해줘. 5줄 이내로."}]
    )
    await app.bot.send_message(chat_id=OWNER_ID, text="📈 이번 주 경제 브리핑\n\n" + response.content[0].text)

async def send_english_study(app):
    response = client.messages.create(
        model="claude-haiku-4-5-20251001",
        max_tokens=512,
        messages=[{"role": "user", "content": "영어 단어 5개와 관용구 2개를 알려줘. 각각 한국어 뜻과 예문 한 줄씩 포함해줘."}]
    )
    await app.bot.send_message(chat_id=OWNER_ID, text="📚 오늘의 영어 공부\n\n" + response.content[0].text)
...
def main():
    ...
    scheduler.add_job(send_news_briefing, CronTrigger(hour=9, minute=0, timezone=KST), args=[app])
    scheduler.add_job(send_economy_briefing, CronTrigger(day_of_week="mon", hour=11, minute=0, timezone=KST), args=[app])
    scheduler.add_job(send_english_study, CronTrigger(hour=12, minute=30, timezone=KST), args=[app])
    scheduler.start()
